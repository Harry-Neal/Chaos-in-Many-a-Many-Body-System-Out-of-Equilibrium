...
int main(){
    ...
    ifstream MyReadFile("dj.txt");
    string myText;
    getline (MyReadFile, myText);
    Jvar = std::stod(myText);
  ...
  //4 Initialize system variables 
	...
	//5.2 evaluate correlators for each position in chain
  	for( int j=0; j<ssize; j++){
		...
		corr[2*j][w_indx] = corr[2*j][t_step] + corrA[j];
		...
		corr[2*j+1][w_indx] = corr[2*j+1][t_step] + corrB[j];
		}
	w_indx = w_indx+1;
	}
	//5.2.5 increment time step
	t_step = t_step + 1;
	//5.4 Propogate copy 1 u;sing Suzuki-Troter decomposition
